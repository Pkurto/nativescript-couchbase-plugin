...
declare var CBLDatabase,
    ...
    CBLQueryMeta,
    ThreadedDatabase,
    ThreadedDatabaseBatchAction;

export class Couchbase extends Common {
    name: string;
    ios: ThreadedDatabase;
    ...
    open(): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            ThreadedDatabase.OpenWithNameCompleteHandler(this.name, (db) => {
                if(!db){
                    return reject('Error initializing database');
                }
                this.ios = db;
                resolve();
            });
    ...
    inBatch(batchActions: BatchAction[]): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            let threadedBatchActions: ThreadedDatabaseBatchAction[];
            threadedBatchActions = [];
            batchActions.forEach(batchAction => {
                if(batchAction === null) {
                    return;
                }
                let threadedBatchAction = new ThreadedDatabaseBatchAction();
                threadedBatchAction.setMutableDocumentWithMutableDoc(batchAction.ios);
                if (batchAction.type === BatchActionType.CREATE) {
                    threadedBatchAction.setTypeWithType(ThreadedDatabaseBatchAction.CreateType);
                } else if (batchAction.type === BatchActionType.UPDATE) {
                    threadedBatchAction.setTypeWithType(ThreadedDatabaseBatchAction.UpdateType);
                } else if (batchAction.type === BatchActionType.DELETE) {
                    threadedBatchAction.setTypeWithType(ThreadedDatabaseBatchAction.DeleteType);
                }
                threadedBatchActions.push(threadedBatchAction);
            });
            if (threadedBatchActions.length > 0) {
                this.ios.inBatchWithActionListCompleteHandler(threadedBatchActions, (error) => {
                    if(error){
                        return reject(error);
                    }
                    resolve();
    ...
    createDocument(data: Object, documentId?: string): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            ...
            this.ios.saveWithMutableDocCompleteHandler(doc, (error) => {
                if(error){
                    return reject(error);
                }
                resolve(doc.id);
    ...
    getDocument(documentId: string): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            this.ios.getDocumentWithDocIdCompleteHandler(documentId, (doc) => {
                if (doc) {
                    let obj = {};
                    const keys = doc.keys;
    ...
    updateDocument(documentId: string, data: any): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            this.ios.getDocumentWithDocIdCompleteHandler(documentId, (original) => {
                if (original) {
                    ...
                    this.ios.saveWithMutableDocCompleteHandler(newDoc, (error) => {
                        if(error){
                            return reject(error);
                        }
                        resolve();
    ...
    deleteDocument(documentId: string): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            this.ios.getDocumentWithDocIdCompleteHandler(documentId, (doc) => {
                if (doc) {
                    this.ios.deleteWithDocumentCompleteHandler(doc, (error) => {
                        if(error){
                            return reject(error);
                        }
                        resolve();
    ...
    destroyDatabase(): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            this.ios.deleteWithCompleteHandler((error) => {
                if(error){
                    return reject(error);
                }
                resolve();
    ...
    addDatabaseChangeListener(callback: any): Promise<any> {
        return new Promise((resolve, reject) => {
            this.ios.addChangeListenerWithListenerCompleteHandler((change: CBLDatabaseChange) => {
    ...
    query(query: Query = {select: [QueryMeta.ALL, QueryMeta.ID]}): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            if (query.from) {
                ...
                from = CBLQueryDataSource.database(this.ios.database);
            }
            ...
            this.ios.executeQueryWithQueryCompleteHandler(queryBuilder, (res, error) => {
                if (error) {
                    console.log('error:', error);
                    return reject(error);
                }
                let result = res.allResults();
